...
def figure_a() -> str:
    ...
    o += figure_title(
        56,
        62,
        "A workspace is one configured assistant",
    ...
    o.append(
        text(
            fx + 44, fy + 7, 'WORKSPACE   "usaxs-analysis"', size=15, fill="#ffffff", weight="bold"
        )
    )
    ...
    bands = [
        (
            "brain",
            "BRAIN",
            "which AI, and how much it can hold",
            [
                (
                    "Profile (provider)",
                    ["which model, which endpoint,", "which key. The brain."],
                    "openRouter  ·  argo-claude",
                ),
                (
                    "Context window",
                    ["how much fits in one turn", "(set on the profile)"],
                    "200 000 tokens",
                ),
            ],
        ),
        (
            ...
            [
                (
                    "System prompt",
                    ["the ROLE it is playing —", "identity and priorities"],
                    '"You are a careful reviewer…"',
                ),
                (
                    "Skills",
                    ["the HOW-TO — procedures and", "conventions it must follow"],
                    "saxs-basics.md",
                ),
                (
                    "Knowledge base (RAG)",
                    ["your own library, indexed so", "it can be searched by meaning"],
        ...
        (
            ...
            [
                (
                    "Source folder(s)",
                    ["where the stuff IS —", "files it may open by name"],
                    "/share1/USAXS_data/…",
                ),
                (
                    "Target folder",
                    ["where results GO —", "reports, figures, scripts"],
                    "~/Documents/Aida/usaxs",
                ),
                (
                    "MCP servers (group)",
                    ["extra tools: pyIrena,", "instrument control, web…"],
                    "group: pyirena-analysis",
                ),
                (
                    "Python interpreter",
                    ["WHICH conda env the code", "it writes actually runs in"],
        ...
        (
            ...
            [
                (
                    "Safety mode",
                    ["confirm = ask before every write;", "relaxed = don't, inside my folders"],
                    "confirm  |  relaxed",
                ),
                (
                    "Command allowlist",
                    ["shell commands that may run", "without asking. Keep it short."],
                    "git status,  git log *",
                ),
                (
                    "Scripting on / off",
                    ["master switch, plus a hard", "timeout on any one run"],
                    "on  ·  30 s ceiling",
                ),
            ],
        ),
    ...
    for key, name, tagline, boxes in bands:
        ...
        o.append(
            text(
                fx + inner_pad + 22 + 11 * len(name) + 16,
                y + 26,
                "— " + tagline,
                size=14,
                fill=INK_SOFT,
...
def figure_b() -> str:
    ...
    o += figure_title(
        56,
        62,
        "What the model actually sees on every turn",
    ...
    groups = [
        (
            "#3f5fbf",
            "THE SYSTEM MESSAGE  —  assembled from your configuration, in this order",
            [
                (
                    "1",
                    "Identity",
                    '"Your name is Aida" + the personal context you wrote once',
                    "Settings",
                ),
                ...
                (
                    "3",
                    "Operating facts",
                    "your folders, the safety mode, each MCP server's own briefing",
                    "Workspace + MCP",
                ),
        ...
        (
            ...
            [
                (
                    "5",
                    "Tool schemas",
                    "~10 000 tokens for pyIrena — sent every turn, called or not",
                    "MCP group",
                ),
                (
                    "6",
                    "Retrieved passages",
                    "this turn only, if a knowledge base matched your question",
                    "Knowledge base",
                ),
                (
                    "7",
                    "Conversation history",
                    "your messages, its replies, and every tool result so far",
                    "This chat",
                ),
                ...
                (
                    ...
                    "max_tokens — set aside before a word is generated",
                    "Profile",
                ),
            ],
        ),
    ...
    mid = (ytop + yend) / 2
    o.append(
        f'<text transform="translate({kx + 52},{mid}) rotate(-90)" text-anchor="middle" '
    ...
    notes = [
        (
            "Tool schemas are rent, not usage.",
            "Every enabled server costs tokens each turn even if nothing calls it. That is what MCP groups are for.",
        ),
        (
            "Tool results stay in the history.",
            "Twenty fits, each returning a table, is what fills a window — not your prose.",
        ),
        (
            "Prompts and skills are cheap.",
            "A few hundred tokens, and they are what makes the agent behave. Be generous.",
        ),
    ]
    cw = (W - 112 - 40) / 3
    for i, (b1, b2) in enumerate(notes):
        ...
        for wd in words:
            if len(ln) + len(wd) > 52:
                ...
                ln = (ln + " " + wd).strip()
        lines.append(ln)
        for j, wrapped in enumerate(lines[:2]):
            o.append(text(x, ty + 54 + j * 18, wrapped, size=13, fill=INK_SOFT))
    return svg_doc(W, H, "\n".join(o))
...
def figure_c() -> str:
    ...
    o += figure_title(
        56,
        62,
        "Same ten slots — three workspaces",
...
def figure_d() -> str:
    ...
    o += figure_title(
        56,
        62,
        "If you are coming from Claude Desktop or ChatGPT",
        "Most of AIDA is a word you already know. Two rows have no equivalent at all — and those two are the point.",
    )

    rows = [
        (
            "The app is the model",
            "Profile",
            "You choose the model and the endpoint; several profiles can share one endpoint.",
            False,
        ),
        ...
        (
            'A "GPT" or a Project',
            "Workspace",
            "Also carries folders, safety rules, and an interpreter.",
            False,
        ),
        (
            "Uploaded project files",
            "Knowledge base  or  source folder",
            "Two different things: searched by meaning vs. opened by name.",
            False,
        ),
        (
            "Connectors / extensions",
            "MCP servers",
            "Same protocol — a claude_desktop_config.json imports directly.",
            False,
        ),
        (
            '"Allow this action?" popups',
            "Safety mode",
            "You decide how often it asks, per workspace.",
            False,
        ),
        (
            "— nothing like it —",
            "Source / target folders",
            "The agent reads and writes real files on your disk.",
            True,
        ),
        (
            "— nothing like it —",
            "Python interpreter",
            "The agent writes and runs code in your own conda environment.",
            True,
        ),
    ]
...
def main() -> None:
    ...
    for name, fn in FIGURES.items():
        ...
        if args.png:
            ...
            cairosvg.svg2png(
                bytestring=svg.encode(), write_to=str(out / f"{name}.png"), scale=args.scale
            )
