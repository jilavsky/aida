# AIDA — design reference

The enduring design of AIDA: what it is for, the decisions behind it, the
architecture and its hard rules, the configuration model, the safety model,
the testing philosophy, the dependency policy, and how it is distributed.

This was sections 1–9 of `PLAN.md` up to the 0.1.0b1 beta. It moved here
when `PLAN.md` was cut down to open work only: none of it is a to-do list,
all of it is still true, and it is the document to read (with the
per-phase files in this folder) to understand *why* the code is shaped the
way it is. Changes to a decision recorded here still get a dated note
appended to §2, as they always did.

- Repo: `https://github.com/jilavsky/Aida`
- Import package `aida`; PyPI distribution `aida-workbench`; console
  scripts `aida` / `aida-gui`
- License MIT; Python >= 3.11; PySide6 desktop GUI
