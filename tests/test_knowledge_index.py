...
def test_replace_file_chunks_inserts_and_records_profile(tmp_path: Path):
    conn = kb_index.connect(tmp_path / "kb.db")
    chunks = [Chunk(text="alpha", heading="A", chunk_index=0), Chunk(text="beta", heading="A", chunk_index=1)]
    embeddings = [[1.0, 0.0], [0.0, 1.0]]

    kb_index.replace_file_chunks(
        conn, source_path="/docs/a.md", mtime=100.0,
        chunks_with_embeddings=list(zip(chunks, embeddings, strict=True)), embedding_profile="mock",
    )
...
def test_replace_file_chunks_deletes_the_files_old_chunks_first(tmp_path: Path):
    ...
    kb_index.replace_file_chunks(
        conn, source_path="/docs/a.md", mtime=100.0,
        chunks_with_embeddings=list(zip(old_chunk, [[1.0, 0.0]], strict=True)), embedding_profile="mock",
    )
    new_chunk = [Chunk(text="new content", heading=None, chunk_index=0)]
    kb_index.replace_file_chunks(
        conn, source_path="/docs/a.md", mtime=200.0,
        chunks_with_embeddings=list(zip(new_chunk, [[0.0, 1.0]], strict=True)), embedding_profile="mock",
    )
...
def test_delete_source_removes_only_that_files_chunks(tmp_path: Path):
    ...
    kb_index.replace_file_chunks(
        conn, source_path="/docs/a.md", mtime=100.0,
        chunks_with_embeddings=[(Chunk(text="a", heading=None, chunk_index=0), [1.0])], embedding_profile="mock",
    )
    kb_index.replace_file_chunks(
        conn, source_path="/docs/b.md", mtime=100.0,
        chunks_with_embeddings=[(Chunk(text="b", heading=None, chunk_index=0), [1.0])], embedding_profile="mock",
    )
...
def test_indexed_source_mtimes_one_entry_per_file(tmp_path: Path):
    conn = kb_index.connect(tmp_path / "kb.db")
    kb_index.replace_file_chunks(
        conn, source_path="/docs/a.md", mtime=111.0,
...
def test_embedding_profile_reflects_most_recent_build(tmp_path: Path):
    conn = kb_index.connect(tmp_path / "kb.db")
    kb_index.replace_file_chunks(
        conn, source_path="/docs/a.md", mtime=1.0,
        chunks_with_embeddings=[(Chunk(text="a", heading=None, chunk_index=0), [1.0])], embedding_profile="profile-1",
    )
    assert kb_index.embedding_profile_used(conn) == "profile-1"
    kb_index.replace_file_chunks(
        conn, source_path="/docs/b.md", mtime=1.0,
        chunks_with_embeddings=[(Chunk(text="b", heading=None, chunk_index=0), [1.0])], embedding_profile="profile-2",
    )
